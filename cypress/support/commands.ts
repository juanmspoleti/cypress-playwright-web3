...
import { getNetwork } from "./utils/contractsData"
import { CustomBridge } from "./utils/CustomBridge"

export interface IWalletLoginOptions {
  rpcUrl?: string
}

Cypress.Commands.add(
  "loginWithWallet",
  ({ rpcUrl = getNetwork().rpcUrl }: IWalletLoginOptions = {}) => {
    cy.on("window:before:load", (win) => {
      const provider = new ethers.JsonRpcProvider(rpcUrl)
      const privateKey = Cypress.env("cypress_wallet_private_key")
      const signer = new ethers.Wallet(privateKey, provider)

      // inject ethereum object in the global window
      const customBridge = new CustomBridge(signer as any, provider)
      ...
      win.addEventListener("eip6963:requestProvider", (event: "eip6963:requestProvider") => {
        announceProvider()
      })
      announceProvider()
    })
...
declare global {
  namespace Cypress {
    interface Chainable {
      /**
       * Login using wallet.
       * @param {String} options.rpcUrl - RPC provider URL to use.
       * @example cy.loginWithWallet({rpcUrl: getNetwork("1").rpcUrl})
       */
      loginWithWallet: (options?: IWalletLoginOptions) => void
    }
  }
}
